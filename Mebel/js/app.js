...
let circles = document.querySelector('.factory__backgr img');
...
body.addEventListener('mousemove', function(evt){
   ...
   var posX2 = (-110 - evt.clientX / w * startX)
   var posY2 = (-15 - evt.clientY / h * startY)
...
const headerContent = document.querySelector(".header");
   const headerFirst = document.querySelectorAll(".factory");
...
const cover = document.querySelectorAll('.fg-cover')
const container = document.querySelectorAll('.fg-container')
let scrollY;
...
function handleScroll () {
  ...
  for(let i = 0;  i < cover.length; i++){
	  cover[i].style.transform = `translateY(-${scrollY}px)`
  }
}
...
// Lazy load
let images = document.querySelectorAll('.tabs-block_load');
...
let tabsBlock = document.querySelectorAll('.tabs-block__box');
tabsBlock.forEach((tabstor) => {
   document.addEventListener('click', function (e){
      e = e.target;
      if(e.classList.contains('tabs-block__union') || e.classList.contains('tabs-block__button')){
         console.log(e)
      }else{
         if(e.classList.contains('tabs-block_load')){
            tabstor.classList.remove('_open')
            e.parentNode.classList.add('_open')
         }else{
            tabstor.classList.remove('_open')
         }
      }
...
document.addEventListener('click', function(e){
   e = e.target;
   if(e.classList.contains('tabs-block__close')){
      if(e.parentNode.classList.contains('_open')){
         e.parentNode.classList.remove('_open');
      }
   }
})
...
for(let anchor of anchors){
   anchor.addEventListener("click", function(event) {
      ...
      const blockID = anchor.getAttribute('top');
